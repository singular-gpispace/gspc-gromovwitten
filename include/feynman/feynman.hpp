...
std::vector<fmpq_t*> feynman_integral_degree_sum(std::vector<std::pair<int, int>>& Gv, const int& d, const std::vector<int>& g = std::vector<int>(), const std::vector<int>& l = std::vector<int>());
#endif // FEYNMAN_HPP