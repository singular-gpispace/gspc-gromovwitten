...
namespace feynman
{
  auto const error_inconsistent_type{ [](auto const& value, auto const& type_description)
                                     {
                                       throw std::logic_error((boost::format("Inconsistency: Expected type '%1%'. "
                                                                             "Got value '%2%' with signature '%3%'.") %
                                                               type_description % pnet::type::value::show(value) % pnet::type::signature::show(pnet::signature_of(value)))
                                                                  .str());
                                     } };

  void WorkflowResult::assert_key_count(Key key, std::size_t expected_count) const
  {
    auto const count(_values_on_ports.count(key));

    if (count != expected_count)
    {
      throw std::logic_error(str(boost::format("Expected count '%1%' for key '%2%': "
        "Got count '%3%' in { %4% }") %
        expected_count % key % count % fhg::util::join(_values_on_ports, ",", [](auto& os, auto const& kv) -> decltype(os)
          { return os << kv.first
          << " = "
  ...
  template <typename T, typename TypeDescription>
  std::vector<std::string> WorkflowResult::get_all_impl(Key key, TypeDescription type_description, std::size_t expected_count) const
  {
    ...
    std::transform(range.first, range.second, std::back_inserter(values), [&type_description](auto const& key_and_value)
      {
  ...
  template <>
  std::vector<std::string> WorkflowResult::get_all(Key key, std::size_t expected_count) const
  {
