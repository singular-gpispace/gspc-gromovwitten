...
namespace feynman
{
  ...
  T const &WorkflowResult::get_impl(Key key, TypeDescription type_description) const
  {
    ...
    {
      throw std::logic_error(str(boost::format("Inconsistency: Expected type '%1%'. Got value '%2%' with signature '%3%'.") % type_description % pnet::type::value::show(value) % pnet::type::signature::show(pnet::signature_of(value))));
    }
  ...
  template <>
  std::string const &WorkflowResult::get(Key key) const
  {
