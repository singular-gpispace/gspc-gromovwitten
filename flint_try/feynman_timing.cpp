...
int main()
{
   ...
   // std::vector<std::pair<int, int>> Gv = { {1, 2}, {1, 2}, {1, 3}, {2, 4}, {3, 4}, {3, 5}, {4, 6}, {5, 6}, {5, 6} };
    ...
    std::vector<std::pair<int, int>> Gv = { {1, 2}, {1, 2}, {1, 2} };
       ...
       for (std::vector<int> x : gen)
       {
           ...
           for (std::vector<int> av : it)
           {
               ...
               for (std::tuple<int, std::vector<int>> tuple : sgn)
               {
                   ...
                   unsigned long fe = feynman_integral_type(Gv, factor, a);
                   fey_degree.push_back(fe);
               }
           }
       }
       unsigned long result = std::accumulate(fey_degree.begin(), fey_degree.end(), static_cast<unsigned long>(0));
   */
    unsigned long result = feynman_integral_degree(Gv, d);
