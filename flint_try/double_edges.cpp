...
unsigned long feynman_integral_degree1(const std::vector<std::pair<int, int>>& F, int d) {
    auto indices = find_equal_pairs(F);

    if (indices.empty()) {
        return feynman_integral_degree(F, d);
    }
    else {
        std::vector<std::vector<int>> re;
        std::vector<unsigned long> res;
        auto L = compos_iterate(F.size(), d);

        while (!L.empty()) {
            auto ll = L.front();
            L.erase(L.begin());

            bool found = false; // Placeholder for actual logic

            if (!found) {
                auto ge = generate_permutation(ll, indices);
                // Removing ge from L
                std::set<std::vector<int>> L_set(L.begin(), L.end());
                for (const auto& perm : ge) {
                    L_set.erase(perm);
                }
                ...
                if (ge.size() == 1) {
                    auto kk = ge[0];
                    res.push_back(feynman_integral_branch_type(F, kk));
                }
                else {
                    auto k = ge[0];
                    unsigned long f = feynman_integral_branch_type(F, k);
                    if (f != 0) {
                        unsigned long ss = ge.size();
                        res.push_back(f * ss);
        ...
        if (res.empty()) {
            ...
            // Sum of results
            unsigned long  myfey = std::accumulate(res.begin(), res.end(), 0UL);
...
int main() {
    std::vector<std::pair<int, int>> F = { {1, 3}, {1, 2}, {1, 2}, {2, 4}, {3, 4}, {3, 4} };
    // std::vector<std::pair<int, int>> F = { {1, 2}, {1, 3}, {1, 4}, {2, 3}, {2, 4}, {3, 4} };
     //std::vector<std::pair<int, int>> F = { {1,2},{1,2},{1,3},{2,4},{3,4},{3,5},{4,6},{5,6},{5,6} };
    // std::vector<std::pair<int, int>> F = { {1,2},{1,2},{1,3},{2,4},{3,4},{3,5},{4,6},{5,6},{5,7},{6,8},{7,8},{7,8} };

    int d = 14;

    // Calculate Feynman integral degree
    unsigned long result = feynman_integral_degree1(F, d);
    std::cout << "Feynman integral degree: " << result << std::endl;
