...
signature iterate_permutation(const graph& ve, const std::vector<int>& a) {
    ...
    for (const auto& ai : L) {
        if (std::find(gg.begin(), gg.end(), ai) == gg.end()) {
            ...
            res.push_back({ static_cast<int>(ge.size()), ai });
...
unsigned long fey_deg(const std::vector<std::pair<int, int>>& ve, int d) {
    ...
    for (const auto& a : gen) {
        ...
        signature itr = iterate_permutation(ve, a);
        ...
        for (const auto& pair : itr) {
            int coef = pair.first;
            const std::vector<int>& ai = pair.second;
...
int main() {
   ...
   // std::vector<std::pair<int, int>> ve = { {1,3}, {1,2},{1,2},{2,4},{3,4},{3,5},{4,6},{5,6},{5,7},{6,8},{7,8},{7,8} };
    ...
    /* IterateResult result = iterate_permutation(F, a);

    if (result.is_vector2d) {
        std::cout << "Returned vector2d: \n";
        for (const auto& vec : result.vector2d) {
            for (int num : vec) {
                std::cout << num << " ";
            }
            std::cout << "\n";
        ...
        std::cout << "Returned pairs: \n";
        for (const auto& pair : result.pairs) {
            std::cout << "Size: " << pair.first << " Vector: ";
 ...
 */
    unsigned long resu = feynman_degree(F, d);
