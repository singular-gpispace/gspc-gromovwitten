...
bool m(const Sequence &u, std::vector<int> &v)
{
    ...
    return std::all_of(v.begin(), v.end(), [](int val)
                       { return val == 0; });
}
...
/*
int feynman_integral(const FeynmanGraph& G, const std::vector<int>& a) {
    std::vector<Edge> ee = G.edges();
...
/*int feynman_integral_b(FeynmanGraph& G, const std::vector<int>& a, const std::vector<int>& l = std::vector<int>()) {
    std::vector<Edge> ee = G.edges();
